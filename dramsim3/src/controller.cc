...
Controller::Controller(int channel, const Config &config, const Timing &timing)
    ...
    : channel_id_(channel),
      ...
      last_trans_clk_(0),
      write_draining_(0),
      app(config.adaptive_page_policy),
      app_hitrate_th(config.rowbuf_hitrate_th),
      app_psel_ctr(config.app_psel_max/2) {
...
void Controller::ClockTick() {
    ...
    channel_state_.UpdateRFMStatus(clk_);

    // Impress-Integer: count row-open-for-tRC as ACT
    if ((config_.impress_d_enable) && (clk_ % config_.tRC == 0 && clk_ != 0)) {
        for (int i = 0; i < config_.ranks; i++) {
            for (int j = 0; j < config_.bankgroups; j++) {
                for (int k = 0; k < config_.banks_per_group; k++) {
                    auto prev_row = channel_state_.GetImpressOpenRow(i, j, k);
                    auto curr_row = channel_state_.OpenRow(i, j, k);
                    if ((prev_row == curr_row) && prev_row != -1) {
                        // std::cout << "ID-ACT sent " << " ch " << channel_id_ 
                                    // << " ra " << i << " ba " << (8*k + j) 
                                    // << " ro " << prev_row << " tON " 
                                    // << config_.tRC << std::endl;
                        act_queue_.push_back(Address(channel_id_, i, 0, (8*k + j), 
                                                    prev_row, 0, config_.tRC));
                    }
                    channel_state_.UpdateImpressOpenRow(i, j, k, curr_row);
                }
            }
        }
    }
    ...
    if (channel_state_.IsRefreshWaiting()) {
        ...
        if (cmd.cmd_type == CommandType::REFRESH) {
            // Send back refresh info for RFM
            // std::cout << "DRAMSim REF to " << "ch " << channel_id_
            //             << " ra " << cmd.Rank() << " ba " << (8*cmd.Bank() + cmd.Bankgroup()) << std::endl;
            act_queue_.push_back(Address(channel_id_, cmd.Rank(), 0, -1, -1, 0));
        }
    }
...
void Controller::iRFMToBank(int ba, int ra) {
    int bank = ba/8;
    int bg = ba - 8*bank;
    channel_state_.BankNeedRFM(bank, bg, ra, clk_);
    simple_stats_.Increment("num_rfm_cmds");
}
...
void Controller::ScheduleTransaction() {
    ...
    for (auto it = queue.begin(); it != queue.end(); it++) {
        ...
        if (config_.adaptive_page_policy && cmd.Row() % 17 == 0) {
            // Samplers are always open page
            cmd.cmd_type = cmd.IsRead() ? CommandType::READ : 
                                          CommandType::WRITE;
        }
        else if (config_.adaptive_page_policy) {
            // Decide page policy for followers
            if (app_psel_ctr >= config_.app_psel_max/2) {
                // Open Page
                cmd.cmd_type = cmd.IsRead() ? CommandType::READ : 
                                              CommandType::WRITE;
            }
...
Command Controller::TransToCommand(const Transaction &trans) {
    ...
    if (trans.isRHMit) {
        simple_stats_.Increment("num_rhmits");
        cmd_type = trans.is_write ? CommandType::WRITE_PRECHARGE
                                  : CommandType::READ_PRECHARGE;
    }
    return Command(cmd_type, addr, trans.addr, trans.isRHMit);
}
...
void Controller::UpdateCommandStats(const Command &cmd) {
    if (config_.adaptive_page_policy && cmd.Row() % 17 == 0) {
        // Update PSEL using samplers
        if (cmd.cmd_type == CommandType::ACTIVATE && app_psel_ctr) {
            // Reduce PSEL ctr on ACTs
            app_psel_ctr--;
        }
        if ((cmd.cmd_type == CommandType::READ || 
            ...
            (channel_state_.RowHitCount(cmd.Rank(), cmd.Bankgroup(),
                                           cmd.Bank()) != 0)) {
            // Increase by X on row hits
            app_psel_ctr += config_.app_psel_inc;
            if (app_psel_ctr > config_.app_psel_max) {
                app_psel_ctr = config_.app_psel_max;
            }
        }
    }
    if (config_.impress_p_enable && (cmd.cmd_type == CommandType::READ_PRECHARGE ||
                            cmd.cmd_type == CommandType::WRITE_PRECHARGE ||
                            cmd.cmd_type == CommandType::PRECHARGE)) {
        int tON;
        int tPRE = config_.tRP;
        int tRC = config_.tRC;
        tON = clk_ - channel_state_.GetLastACTCycles(cmd.Rank(), cmd.Bankgroup(), cmd.Bank());
        if (cmd.cmd_type == CommandType::READ_PRECHARGE) {
            tON += config_.AL + config_.burst_cycle + config_.tRTP;
        }
        else if (cmd.cmd_type == CommandType::WRITE_PRECHARGE) {
            tON += config_.WL + config_.burst_cycle + config_.tWR;
        }
        assert(tON >= config_.tRAS && 
                channel_state_.OpenRow(cmd.Rank(), cmd.Bankgroup(), cmd.Bank()) != -1);
        // std::string curr_cmd = (cmd.cmd_type == CommandType::READ_PRECHARGE) ? "READ_PRECHARGE" :
        //                         ((cmd.cmd_type == CommandType::WRITE_PRECHARGE) ? "WRITE_PRECHARGE" :
        //                         "PRECHARGE");
        // std::cout << "IP-ACT ch " << channel_id_ << " ra " << cmd.Rank() << " ba "
        //             << (8*cmd.Bankgroup() + cmd.Bank()) << " ro " 
        //             << cmd.Row() << " tON " << tON << std::endl;
        act_queue_.push_back(Address(channel_id_, cmd.Rank(), 0, (8*cmd.Bank() + cmd.Bankgroup()), 
    ...
    switch (cmd.cmd_type) {
        ...
        case CommandType::PRECHARGE:
            ...
            if (cmd.expressPRE) {
                simple_stats_.Increment("num_express_pre_cmds");
            }
            break;
