...
bool JedecDRAMSystem::AddTransaction(uint64_t hex_addr, bool is_write, bool isRHMit) {
// Record trace - Record address trace for debugging or other purposes
    ...
    assert(ok);
    if (ok) {
        Transaction trans = Transaction(hex_addr, is_write, isRHMit);
...
void JedecDRAMSystem::iRFMToBank(int ba, int ra, int ch) {
    ctrls_[ch]->iRFMToBank(ba, ra);
}
...
void JedecDRAMSystem::ClockTick() {
    for (size_t i = 0; i < ctrls_.size(); i++) {
        ...
        while (true) {
            ...
            if (actAddr.channel != -1 && actAddr.rank != -1) {
                // if (actAddr.tON != -1) {
                    // std::cout << "ACT-callback sent ch " << actAddr.channel
                    //             << " ra " << actAddr.rank << " ba " << actAddr.bank
                    //             << " ro " << actAddr.row << " tON " << actAddr.tON << std::endl;
                // }
                act_callback_(actAddr.channel, actAddr.rank,
                                actAddr.bank, actAddr.row,
                                actAddr.tON, actAddr.tPRE, actAddr.tRC);
            }
            else {
                break;
            }
...
bool IdealDRAMSystem::AddTransaction(uint64_t hex_addr, bool is_write, bool isRHMit) {
