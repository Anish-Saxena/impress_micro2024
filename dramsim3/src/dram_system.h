...
class BaseDRAMSystem {
   public:
    ...
    virtual bool AddTransaction(uint64_t hex_addr, bool is_write, bool isRHMit) = 0;
    virtual void iRFMToBank(int ba, int ra, int ch) = 0;
    virtual void ClockTick() = 0;
    int GetChannel(uint64_t hex_addr) const;

    std::function<void(uint64_t req_id)> read_callback_, write_callback_;
    std::function<void(uint64_t ch, uint64_t ra, 
                    uint64_t ba, uint64_t ro, uint64_t tON,
                    uint64_t tPRE, uint64_t tRC)> act_callback_;
...
class JedecDRAMSystem : public BaseDRAMSystem {
   public:
    ...
    bool AddTransaction(uint64_t hex_addr, bool is_write, bool isRHMit) override;
    void iRFMToBank(int ba, int ra, int ch) override;
