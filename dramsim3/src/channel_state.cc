...
ChannelState::ChannelState(const Config& config, const Timing& timing)
    ...
    for (auto i = 0; i < config_.ranks; i++) {
        ...
        for (auto j = 0; j < config_.bankgroups; j++) {
            auto bg_states =
                std::vector<BankState>(config_.banks_per_group, BankState(config.tONMAX, config.tRFM));
...
void ChannelState::BankNeedRFM(int ba, int bg, int ra, uint64_t clk) {
    bank_states_[ra][bg][ba].BankNeedRFM(clk);
}

bool ChannelState::IsBankInRFM(int ba, int bg, int ra, uint64_t clk) const {
    return bank_states_[ra][bg][ba].IsBankInRFM(clk);
}
...
void ChannelState::UpdateTimingAndStates(const Command& cmd, uint64_t clk) {
    UpdateState(cmd);
    // Impress: update last activation time for the bank
    if (cmd.cmd_type == CommandType::ACTIVATE) {
        int rank = cmd.Rank();
        int bankgroup = cmd.Bankgroup();
        int bank = cmd.Bank();
        bank_states_[rank][bankgroup][bank].UpdateLastACTInfo(clk);
    }
