...
void Config::CalculateSize() {
    ...
    std::cout << "Adaptive open page? "  << adaptive_page_policy 
                << " PSEL_th " << app_psel_max/2 
                << " RBHR_th " << 1/(app_psel_inc+1) << std::endl;
    std::cout << "Impress-D enabled? " << impress_d_enable 
                << " Impress-P enabled? " << impress_p_enable << std::endl;
    std::cout << "MOP gang size " << mop_gang_size << std::endl;
...
void Config::InitSystemParams() {
    ...
    adaptive_page_policy = reader.GetBoolean("system", "adaptive_page_policy", true);
    app_psel_max = reader.GetInteger("system", "app_psel_max", 1023);
    app_psel_inc = reader.GetInteger("system", "app_psel_inc", 15);
    rowbuf_hitrate_th = reader.GetReal("system", "rowbuf_hitrate_th", 0.25);
    impress_d_enable = reader.GetBoolean("system", "impress_d_enable" , false);
    impress_p_enable = reader.GetBoolean("system", "impress_p_enable" , false);
    mop_gang_size = reader.GetInteger("system", "mop_gang_size", 0);
    ...
    in_dram_rfm = reader.GetBoolean("system", "in_dram_rfm", false);
    return;
}
...
void Config::InitTimingParams() {
    ...
    // Impress
    tONMAX = GetInteger("timing", "tONMAX", -1);
    tRFM = GetInteger("timing", "tRFM", 528);
    ...
    // Impress
    if (tONMAX == -1) {
        std::cout << "[IMPRESS] tONMAX DISABLED" << std::endl;
    }
    else {
        std::cout << "[IMPRESS] tONMAX ENABLED and equals " << tONMAX * tCK << "ns" << std::endl;
        if (queue_structure != "PER_BANK") {
            std::cout << "Rank-level queue not supported with tONMAX! Exiting..." << std::endl;
            AbruptExit(__FILE__, __LINE__);
        }
    }
    return;
...
void Config::SetAddressMapping() {
    ...
    if (mop_gang_size) {
        std::cout << "MOP address mapping enabled: rohirababgchlo" << std::endl;
        int gang_bits = LogBase2(mop_gang_size);
        field_widths["hi"] = actual_col_bits - gang_bits;
        field_widths["lo"] = gang_bits;
    ...
    if (address_mapping.size() != 12 && !mop_gang_size) {
