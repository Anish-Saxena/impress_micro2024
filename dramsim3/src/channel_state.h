...
class ChannelState {
   public:
    ...
    bool IsBankInRFM(int ba, int bg, int ra, uint64_t clk) const;
    void BankNeedRFM(int ba, int bg, int ra, uint64_t clk);
    void UpdateRFMStatus(int clk) {
        for (int i = 0; i < config_.ranks; i++) {
            for (int j = 0; j < config_.bankgroups; j++) {
                for (int k = 0; k < config_.banks_per_group; k++) {
                    bank_states_[i][j][k].UpdateRFMStatus(clk);
                }
            }
        }
    }
    ...
    // Impress
    bool IsExpressPRENeeded(int rank, int bg, int bank, uint64_t clk) const {
        return bank_states_[rank][bg][bank].ExpressPRENeeded(clk);
    }

    uint64_t GetImpressOpenRow(int rank, int bg, int bank) {
        return bank_states_[rank][bg][bank].GetImpressOpenRow();
    }

    void UpdateImpressOpenRow(int rank, int bg, int bank, int row) {
        return bank_states_[rank][bg][bank].UpdateImpressOpenRow(row);
    }

    uint64_t GetLastACTCycles(int rank, int bg, int bank) {
        return bank_states_[rank][bg][bank].GetLastACTCycles();
    }

    // bool SampleACCAct(int rank, int bg, int bank, int option) {
    //     return bank_states_[rank][bg][bank].SampleACCAct(option);
    // }
