...
class CommandQueue {
   ...
   private:
    ...
    bool HasRWDependency(const CMDIterator& cmd_it,
                         const CMDQueue& queue) const;
    Command GetFirstReadyInQueue(CMDQueue& queue, int q_idx) const;
    ...
    // Impress
    int GetRankFromQIdx(int q_idx) const;
    int GetBGFromQIdx(int q_idx) const;
    int GetBankFromQIdx(int q_idx) const;
