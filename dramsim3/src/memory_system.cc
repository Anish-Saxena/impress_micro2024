...
bool MemorySystem::AddTransaction(uint64_t hex_addr, bool is_write, bool isRHMit) {
    return dram_system_->AddTransaction(hex_addr, is_write, isRHMit);
}

void MemorySystem::iRFMToBank(int ba, int ra, int ch) {
    dram_system_->iRFMToBank(ba, ra, ch);
}
