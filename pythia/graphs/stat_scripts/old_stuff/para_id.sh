...
CURR_DIR=`pwd`
OUT_FILE="${CURR_DIR}/data/para_id_10M.csv"
...
suite1=`python get_norm_ipc.py ${suite1_DIR} DETAILED_OUTPUT para_drfm_T1000/no_mro/`
suite2=`python get_norm_ipc.py ${suite2_DIR} DETAILED_OUTPUT para_drfm_T1000/no_mro/`
suite3=`python get_norm_ipc.py ${suite3_DIR} DETAILED_OUTPUT para_drfm_T1000/no_mro/`
...
paste <(echo "$BMK_NAMES") <(echo "$suite1") <(echo "$suite2") <(echo "$suite3") <(echo "$suite4") >> ${OUT_FILE}